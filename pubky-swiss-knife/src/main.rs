        ...
        div { class: "tab-body tight",
            ...
            section { class: "card",
                ...
                div { class: "form-grid",
                    label {
                        "Recovery file path"
                        input { value: recovery_path_value.clone(), oninput: move |evt| recovery_path_binding.set(evt.value()) }
                    }
                    label {
                        "Passphrase"
                ...
                div { class: "small-buttons",
                    button { class: "action", onclick: move |_| {
                            ...
                            if raw_path.trim().is_empty() {
                                push_log(load_logs.clone(), LogLevel::Error, "Provide a path to load the recovery file");
                                return;
                            }
                            ...
                            spawn(async move {
                                let outcome = (|| -> Result<(Keypair, PathBuf)> {
                                    let normalized = normalize_pkarr_path(&raw_path)?;
                                    let kp = load_keypair_from_recovery(&normalized, &passphrase)?;
                                    Ok((kp, normalized))
                                })();
                                match outcome {
                                    Ok((kp, normalized)) => {
                    ...
                    button { class: "action secondary", onclick: move |_| {
                            ...
                            if raw_path.trim().is_empty() {
                                push_log(save_logs.clone(), LogLevel::Error, "Provide a path to save the recovery file");
                                return;
                            }
                            if let Some(kp) = save_keypair_signal.read().as_ref().cloned() {
                                ...
                                spawn(async move {
                                    match save_keypair_to_recovery_file(&kp, &raw_path, &passphrase) {
                                        Ok(path) => {
