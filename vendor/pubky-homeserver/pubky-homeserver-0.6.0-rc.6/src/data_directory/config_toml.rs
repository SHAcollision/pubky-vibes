...
/// Discovery-related publication settings.
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq)]
pub struct DiscoveryToml {
    /// Interval (in minutes) for refreshing the primary Pkarr packet.
    #[serde(default = "DiscoveryToml::default_pkarr_publish_interval_minutes")]
    pub pkarr_publish_interval_minutes: NonZeroU64,
    /// Configuration for `_iroh` TXT publication.
    #[serde(default)]
    pub iroh: IrohToml,
}

impl DiscoveryToml {
    fn default_pkarr_publish_interval_minutes() -> NonZeroU64 {
...
impl Default for DiscoveryToml {
    fn default() -> Self {
        Self {
            pkarr_publish_interval_minutes: Self::default_pkarr_publish_interval_minutes(),
            iroh: IrohToml::default(),
        }
    }
}

/// Settings controlling how `_iroh` TXT records are emitted.
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq)]
pub struct IrohToml {
    /// Whether `_iroh` TXT publication is enabled.
    #[serde(default)]
    pub enabled: bool,
    /// Optional relay URL announced to clients.
    #[serde(default)]
    pub relay_url: Option<Url>,
    /// Direct socket addresses reachable without a relay.
    #[serde(default)]
    pub direct_addresses: Vec<String>,
    /// Application protocol identifier used on the QUIC tunnel.
    #[serde(default = "IrohToml::default_alpn")]
    pub alpn: String,
    /// TTL applied to generated TXT records.
    #[serde(default = "IrohToml::default_txt_ttl_seconds")]
    pub txt_ttl_seconds: u32,
    /// Publish cadence (in minutes) for `_iroh` records.
    #[serde(default = "IrohToml::default_publish_interval_minutes")]
    pub publish_interval_minutes: NonZeroU64,
}

impl IrohToml {
    fn default_publish_interval_minutes() -> NonZeroU64 {
        NonZeroU64::new(5).expect("non-zero")
    }

    fn default_txt_ttl_seconds() -> u32 {
        300
    }

    fn default_alpn() -> String {
        "pubky/iroh-homeserver/0".to_string()
    }
}

impl Default for IrohToml {
    fn default() -> Self {
        Self {
            enabled: false,
            relay_url: None,
            ...
            txt_ttl_seconds: Self::default_txt_ttl_seconds(),
            publish_interval_minutes: Self::default_publish_interval_minutes(),
        }
    }
}
...
pub struct ConfigToml {
    ...
    /// Settings driving discovery publication behaviour.
    #[serde(default)]
