...
use crate::iroh::gateway::IrohGateway;
...
pub struct HomeserverCore {
    ...
    #[allow(dead_code)]
    pub(crate) iroh_gateway: Option<IrohGateway>,
...
impl HomeserverCore {
    ...
    pub async fn new(context: AppContext) -> std::result::Result<Self, HomeserverBuildError> {
        ...
        let iroh_gateway = if context.config_toml.discovery.iroh.enabled {
            match IrohGateway::start(&context, icann_http_socket).await {
                Ok(gateway) => Some(gateway),
                Err(error) => {
                    tracing::warn!(?error, "Failed to start embedded Iroh gateway");
                    None
                }
            }
        } else {
        ...
        let gateway_handle = iroh_gateway.as_ref().map(|gateway| gateway.handle());
